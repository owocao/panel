...
export const createBookmarkFolder = (payload) => api('/api/bookmark/folders', jsonRequest('POST', payload))
export const updateBookmarkFolder = (payload) => api('/api/bookmark/folders', jsonRequest('PUT', payload))
export const deleteBookmarkFolder = (id) => api(`/api/bookmark/folders?id=${id}`, { method: 'DELETE' })
...
export async function uploadAsset(file) {
  ...
  const body = await response.json().catch(() => ({}))
  if (!response.ok || body.success === false) throw new Error(body.error || `上传失败：${response.status}`)
  return body.data
}
export async function importBookmarkHTML(file) {
  ...
  const body = await response.json().catch(() => ({}))
  if (!response.ok || body.success === false) throw new Error(body.error || `导入失败：${response.status}`)
  return body.data
}
