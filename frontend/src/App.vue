...
const dragState = ref({ type: '', groupId: null, item: null, overId: null, saving: false })
...
function startDrag(type, item, groupId = null, event = null) {
  dragState.value = { type, item, groupId, overId: null, saving: false }
...
function clearDragState() {
  dragState.value = { type: '', groupId: null, item: null, overId: null, saving: false }
}

function hoverNavCard(group, target) {
  if (dragState.value.type !== 'navItem' || dragState.value.groupId !== group.id) return
...
async function dropNavCard(group, target) {
  const source = dragState.value.item
  if (dragState.value.type !== 'navItem' || dragState.value.groupId !== group.id || !source || source.id === target.id) {
    clearDragState()
    return
  }
  ...
  const reordered = list.map((item, index) => ({ ...item, sort: index + 1 }))
  ...
  <main class="shell sun-shell" @click="handleShellClick">
    ...
    <template v-else>
      ...
      <section v-if="activeView === 'home'" class="home-panel sun-panel">
        ...
        <section v-for="group in displayGroups" :key="group.id || group.name" class="nav-group" :class="{ editing: editingNavGroupId === group.id, 'drag-saving': dragState.saving && dragState.groupId === group.id }" :draggable="!!group.id" @dragstart="group.id && startDrag('navGroup', group, null, $event)" @dragend="clearDragState" @dragover.prevent @drop="group.id && dropNavGroup(group)"><header class="group-head" @contextmenu="showGroupMenu($event, group)"><h2>{{ group.name }}<small v-if="editingNavGroupId === group.id">排序模式</small></h2><div class="group-tools"><button type="button" title="新增卡片" @click="addCardFromMenu(group)"><img :src="iconUrl('plus')" alt="" /></button><button type="button" title="编辑分组" @click="toggleNavGroupEdit(group)"><img :src="iconUrl('edit')" alt="" /></button></div></header><div class="card-grid"><a v-for="item in group.items" :key="item.id || item.name" class="app-tile" :class="{ dragging: dragState.type === 'navItem' && dragState.item?.id === item.id, 'drop-target': dragState.overId === item.id }" :href="editingNavGroupId === group.id ? '#' : resolveNavUrl(item)" :draggable="!!item.id && editingNavGroupId === group.id" @dragstart.stop="item.id && startDrag('navItem', item, group.id, $event)" @dragenter.stop.prevent="item.id && hoverNavCard(group, item)" @dragover.stop.prevent="item.id && hoverNavCard(group, item)" @dragend.stop="clearDragState" @drop.stop.prevent="item.id && dropNavCard(group, item)" @click="openCardEditorInGroup($event, group, item)" @contextmenu="showCardMenu($event, item)"><span class="nav-card"><span v-if="isImageValue(item.icon)" class="card-icon image-icon"><img :src="item.icon" alt="" /></span><span v-else class="card-text-icon" :class="cardTextClass(item.icon || item.name)">{{ limitText(item.icon || item.name, 5) }}</span></span><span class="card-title">{{ limitText(item.name, 10) }}</span></a></div></section>
      </section>
