...
<template>
  <main class="shell" :style="shellStyle" @click="closeMenu">
    <aside class="mode-rail" aria-label="页面切换">
      <button :class="{ active: activeView === 'home' }" type="button" @click.stop="activeView = 'home'">首页</button>
      <button :class="{ active: activeView === 'login' }" type="button" @click.stop="activeView = 'login'">登录</button>
      <button :class="{ active: activeView === 'setup' }" type="button" @click.stop="activeView = 'setup'">初始化</button>
      <button :class="{ active: activeView === 'settings' }" type="button" @click.stop="activeView = 'settings'">设置</button>
    </aside>
    ...
    <section v-if="activeView === 'home'" class="home-panel">
      <header class="topbar"><button class="drawer-trigger" type="button" @click.stop="openDrawer">收藏夹</button><div class="brand"><div class="logo"><img v-if="settingsForm.logoUrl" :src="settingsForm.logoUrl" alt="Logo" /><span v-else>B</span></div><div><h1>{{ settingsForm.siteTitle || 'biu-panel' }}</h1><p>{{ statusText }}</p></div></div><button class="net-switch" type="button" @click="cycleNetworkMode">{{ networkLabel }}</button></header>
      <section class="hero-card"><div><span class="eyebrow">Personal start page</span><h2>常用入口保持轻快，收藏夹按需打开。</h2></div><button type="button" @click="addNavCard">新增卡片</button></section>
      <div class="quick-create"><input v-model="quickNav.groupName" placeholder="新分组名称" /><button type="button" @click="addNavGroup">新增分组</button><input v-model="quickNav.cardName" placeholder="新卡片名称" /><input v-model="quickNav.url" placeholder="卡片网址" /><button type="button" @click="fillQuickNavMetadata">{{ metadataLoading ? '抓取中' : '自动抓取' }}</button><button type="button" @click="addNavCard">新增到第一个分组</button></div>
      <section v-for="group in displayGroups" :key="group.id || group.name" class="nav-group" :draggable="!!group.id" @dragstart="group.id && startDrag('navGroup', group)" @dragover.prevent @drop="group.id && dropNavGroup(group)"><header class="group-head" @contextmenu="showGroupMenu($event, group)"><h2>{{ group.name }}</h2><div class="inline-actions"><button type="button" @click.stop="showGroupMenu($event, group)">菜单</button><button v-if="group.id" type="button" @click="editNavGroup(group)">编辑分组</button><button v-if="group.id" type="button" @click="moveNavGroup(group, -1)">上移</button><button v-if="group.id" type="button" @click="moveNavGroup(group, 1)">下移</button><button v-if="group.id" type="button" @click="removeNavGroup(group)">删除分组</button></div></header><div class="card-grid"><a v-for="item in group.items" :key="item.id || item.name" class="nav-card" :draggable="!!item.id" @dragstart="item.id && startDrag('navItem', item, group.id)" @dragover.prevent @drop.prevent="item.id && dropNavCard(group, item)" :href="resolveNavUrl(item)" @contextmenu="showCardMenu($event, item)" @touchstart.passive="showCardMenu($event, item)"><span class="card-icon"><img v-if="isImageValue(item.icon)" :src="item.icon" alt="" /><span v-else>{{ (item.icon || item.name).slice(0, 1) }}</span></span><span>{{ item.name }}</span><span v-if="item.id" class="card-actions"><button type="button" @click.prevent="editNavCard(item)">编辑</button><button type="button" @click.prevent="moveNavCard(group, item, -1)">上移</button><button type="button" @click.prevent="moveNavCard(group, item, 1)">下移</button><button type="button" @click.prevent="removeNavCard(item)">删除</button></span></a></div></section>
    </section>

    <section v-else-if="activeView === 'login'" class="center-card auth-card"><span class="eyebrow dark">Login</span><h2>管理员登录</h2><p>{{ statusText }}</p><form class="form-grid" @submit.prevent="submitLogin"><label>账号<input v-model="loginForm.username" /></label><label>密码<input v-model="loginForm.password" type="password" /></label><label class="check-row"><input v-model="loginForm.remember" type="checkbox" /> 记住登录</label><button type="submit">登录</button></form></section>
    <section v-else-if="activeView === 'setup'" class="center-card auth-card"><span class="eyebrow dark">First run</span><h2>首次初始化</h2><p>{{ statusText }}</p><form class="form-grid" @submit.prevent="submitSetup"><label>管理员账号<input v-model="setupForm.username" /></label><label>管理员密码<input v-model="setupForm.password" type="password" placeholder="至少 8 位" /></label><label>确认密码<input v-model="setupForm.confirm" type="password" /></label><button type="submit">创建管理员</button></form></section>
    <section v-else class="settings-panel"><header class="settings-head"><div><span class="eyebrow dark">Settings</span><h2>系统设置</h2></div><button type="button" @click="submitSettings">保存设置</button></header><div class="settings-grid"><section class="setting-card"><h3>站点个性化</h3><label>站点标题<input v-model="settingsForm.siteTitle" /></label><label>Logo 图片<input v-model="settingsForm.logoUrl" placeholder="本地上传地址或图片链接" /></label><label>上传 Logo<input type="file" accept="image/*" @change="uploadIconFile($event, settingsForm, 'logoUrl')" /></label><label>背景图<input v-model="settingsForm.backgroundUrl" placeholder="图片链接" /></label><label>上传背景图<input type="file" accept="image/*" @change="uploadIconFile($event, settingsForm, 'backgroundUrl')" /></label><label>背景色<input v-model="settingsForm.backgroundColor" /></label></section><section class="setting-card"><h3>S3 存储</h3><label class="check-row"><input v-model="settingsForm.s3Enabled" true-value="true" false-value="false" type="checkbox" /> 启用 S3 配置</label><label>Endpoint<input v-model="settingsForm.s3Endpoint" placeholder="https://s3.example.com" /></label><label>Region<input v-model="settingsForm.s3Region" placeholder="auto" /></label><label>Bucket<input v-model="settingsForm.s3Bucket" placeholder="biu-panel" /></label><label>Access Key<input v-model="settingsForm.s3AccessKey" /></label><label>Secret Key<input v-model="settingsForm.s3SecretKey" type="password" /></label><label>上传前缀<input v-model="settingsForm.s3Prefix" placeholder="biu-panel/" /></label><label>公开访问域名<input v-model="settingsForm.s3PublicBase" placeholder="https://cdn.example.com/biu-panel" /></label><label class="check-row"><input v-model="settingsForm.s3PathStyle" true-value="true" false-value="false" type="checkbox" /> Path-style 兼容模式</label><div class="inline-actions"><button type="button" @click="submitSettings">保存 S3 配置</button><button type="button" @click="submitTestS3">测试 S3</button></div></section><section class="setting-card"><h3>内外网判断</h3><label>统一检测地址<input v-model="settingsForm.lanDetectUrl" /></label><label>超时时间 ms<input v-model="settingsForm.lanDetectTimeout" /></label><label class="check-row"><input v-model="settingsForm.autoDetectLan" true-value="true" false-value="false" type="checkbox" /> 启用自动判断</label></section><section class="setting-card"><h3>备份恢复</h3><p>系统备份为 .tar.gz，包含数据库、本地上传文件和版本信息。</p><div class="inline-actions"><button type="button" @click="window.location.href = '/api/backup/download'">下载备份</button><button type="button" @click="submitBackupToS3">备份到 S3</button><label class="file-button">恢复备份<input type="file" accept=".gz,.tgz,application/gzip" @change="restoreBackupFile" /></label></div></section></div></section>

    <section v-if="editDialog.open" class="modal-mask" @click.stop="closeEditDialog">
      <form class="edit-modal" @click.stop @submit.prevent="saveEditDialog">
        ...
        <template v-if="editDialog.type === 'navItem'">
          <label>名称<input v-model="editDialog.form.name" /></label>
          <label>图标文字 / 图片链接<input v-model="editDialog.form.icon" /></label><label>上传图标图片<input type="file" accept="image/*" @change="uploadIconFile($event, editDialog.form, 'icon')" /></label>
          <label>内网地址<input v-model="editDialog.form.lanUrl" /></label>
          <label>外网地址<input v-model="editDialog.form.wanUrl" /></label>
          <label>打开模式<select v-model="editDialog.form.urlMode"><option value="auto">自动</option><option value="lan">强制内网</option><option value="wan">强制外网</option></select></label><button type="button" @click="fillMetadata(editDialog.form)">{{ metadataLoading ? '抓取中' : '自动抓取标题/图标' }}</button>
        </template>
        <template v-if="editDialog.type === 'bookmark'">
          <label>标题<input v-model="editDialog.form.title" /></label>
          <label>网址<input v-model="editDialog.form.url" /></label>
          <label>图标<input v-model="editDialog.form.favicon" /></label><label>上传图标图片<input type="file" accept="image/*" @change="uploadIconFile($event, editDialog.form, 'favicon')" /></label>
          <label>备注<input v-model="editDialog.form.note" /></label><button type="button" @click="fillMetadata(editDialog.form)">{{ metadataLoading ? '抓取中' : '自动抓取标题/图标' }}</button>
        </template>
